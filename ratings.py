...
def chart_data():
    ...
    for race in tqdm(race_query):
        ...
        tooltip_data[school.name][race.date] = "<b>{}</b><br/>{}<br/>{}-{} ({})".format(datetime.datetime.fromtimestamp(race.date/1000.0).strftime("%b %d, %Y"), opponent.name, race.school_score, race.opponent_score, change)
    formatted = []
